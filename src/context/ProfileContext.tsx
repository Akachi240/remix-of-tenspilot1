...
export const ProfileProvider = ({ children }: { children: React.ReactNode }) => {
  const [profiles, setProfiles] = useState<Profile[]>(() => {
    try {
      const stored = JSON.parse(localStorage.getItem('tens-companion-profiles') || '[]');
      if (!Array.isArray(stored)) return [];
      return stored.map((profile: Partial<Profile> & { condition?: string }) => ({
        ...profile,
        id: profile.id || crypto.randomUUID(),
        name: profile.name || 'Unnamed Patient',
        primaryCondition: profile.primaryCondition || profile.condition || '',
        medications: Array.isArray(profile.medications) ? profile.medications : [],
        sessionHistory: Array.isArray(profile.sessionHistory) ? profile.sessionHistory : [],
      })) as Profile[];
    } catch {
      return [];
    }
